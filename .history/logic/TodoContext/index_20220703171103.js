...
const taskActives = (state, action) => {
  switch (action.type) {
    case 'ADD_TASK_DONE':
      return {
        ...state,
...
const [state, dispatch] = React.useReducer(taskActives, initialState);

const handleClickTaskDone = (task) => {
  dispatch({type: 'ADD_TASK_DONE', payload: task.text})
} 

const handleClickTaskUnDone = (task) => {
  dispatch({type: 'ADD_TASK_UNDONE', payload: task.text})
} 
